...
class Service extends AdapterService {
  ...
  async createDB (name) {
    this.schema = name;
    return this.client.createSchema({ schema: name })
      .catch(errorHandler);
  }

  // Instantiate the table
  async createTable (name, idProp) {
    this.table = name;
    this.idProp = idProp || this.idProp;
    return this.client.createTable({
      schema: this.schema,
      table: this.table,
      hashAttribute: this.idProp
    })
      .catch(errorHandler);
  }
  ...
  async _find (params) {
    ...
    const result = await this.query(sqlString).catch(errorHandler);
    return result;
  }

  // async _oldfind (params) {
  //   // clean up the input
  //   const { filters, query, paginate } = this.filterQuery(params);
  //   const q = this.createQuery(params);
  ...
  //   // create the structure response
  //   const qString = q.toString();

  //   // escape if limit is zero
  //   if (filters.$limit === 0) {
  ...
  //   if (paginate && paginate.default) {
  //     const countQuery = q.clone().clearSelect().clearOrder().count(`${this.table}.${this.id} as total`);
...
//   // qString = `
//   // select books.*
//   // from test_schema.books as books
//   // where
//   //   hair = 'brown'
//   //   and eyes in ('blue', 'hazel')
//   //   and (
//   //     (name = 'Doug')
//   //     or (
//   //       eyes = 'blue'
...
//   //   )
//   // order by "name" asc
//   return this.client.query(qString)
//     .then(res => res.data)
//     .catch(e => console.log(e));
//   // and "eyes" in ('blue', 'hazel') and (("name" = 'Doug') or ("eyes" = 'blue' and (("age" >= 18 and "age" < 25) or ("name" = 'Tommy')))) order by "name" asc
//   // `
//   // operation: 'search_by_conditions',
//   // operator: OPERATOR[query.$or] || OPERATOR.default,
//   // // offset:  filters.$skip,
