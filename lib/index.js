...
class Service extends AdapterService {
  constructor (options = {}) {
    const { whitelist = [], config={}} = options;
    super(Object.assign({
      id:'id'
    },options, {
  ...
  validateConfig(){
    let { schema, table, password, username, harperHost } = this.config;
    if(!harperHost){
  ...
  // instantiate the DB
  async createDB (name) {
    let schema = name || this.schema;
    return this.client.createSchema({ schema }).catch(errorHandler);
  }
